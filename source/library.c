...
void library_init(void) {
    /* seed with a few sample books */
    if (book_count != 0) return;
    add_book("Pemrograman C untuk Pemula", "A. Programmer");
    add_book("Algoritma dan Struktur Data", "B. Informatika");
    add_book("Sistem Operasi Dasar", "C. Sistem");
}
...
int add_book(const char *title, const char *author) {
    ...
    b->id = next_id++;
    strncpy(b->title, title, TITLE_MAX-1); b->title[TITLE_MAX-1] = '\0';
    strncpy(b->author, author, AUTHOR_MAX-1); b->author[AUTHOR_MAX-1] = '\0';
    b->available = 1;
    return b->id;
}
...
int remove_book(int id) {
    for (int i = 0; i < book_count; ++i) {
        if (books[i].id == id) {
            /* remove by shifting */
            for (int j = i; j + 1 < book_count; ++j) books[j] = books[j+1];
            --book_count;
            return 0;
        }
    }
...
static void downcase_copy(char *dst, const char *src, size_t n) {
    size_t i;
    for (i = 0; i + 1 < n && src[i]; ++i) dst[i] = (char)tolower((unsigned char)src[i]);
    dst[i] = '\0';
}

int find_books_by_title(const char *query, int *out_ids, int max_out) {
    if (!query || !out_ids || max_out <= 0) return 0;
    char qlow[TITLE_MAX];
    downcase_copy(qlow, query, sizeof(qlow));
    int found = 0;
    for (int i = 0; i < book_count && found < max_out; ++i) {
        char tlow[TITLE_MAX];
        downcase_copy(tlow, books[i].title, sizeof(tlow));
        if (strstr(tlow, qlow) != NULL) {
            out_ids[found++] = books[i].id;
        }
    }
    return found;
}
